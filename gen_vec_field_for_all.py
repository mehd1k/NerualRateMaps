...
class MyApp(ShowBase):  # Our modified class for the control loop
    def __init__(self, hd):
        ...
        self.camera.setPos(self.X_all[0], self.Y_all[0], Z)
        self.camera.setHpr(self.hd, P, 0)
    ...
    def gen_grid_points(self, num_points = 10):
        # delta = 0.08
        # ####First grid
        # xmin0 , xmax0 = 0+delta, 0.6-delta
        # ymin0, ymax0 = 0+delta, 1.2-delta
        ...
        # xmin1 , xmax1 = 0.6+delta, 1.2-delta
        # ymin1, ymax1 = 0+delta, 0.6-delta
        ...
        # x_range0  = np.linspace(self.xmin, self.xmax,5)
        # y_range0 = np.linspace(self.ymin, self.ymax,5)
        
        # self.X_all, self.Y_all= np.meshgrid(x_range0, y_range0)
        # self.X_all, self.Y_all = self.X_all.flatten(), self.Y_all.flatten()
        self.X_all, self.Y_all = [0.01,0.01],[0.59,0.59]
    ...
    def gen_vec_field(self, task):
        if self.current_step < self.num_steps:
            ...
            print('camera_position', self.X_all[self.current_step], self.Y_all[self.current_step])
            ...
            next_img = image[:, :, 2]
            plt.imsave('test/img/xyhd'+'_'+str(self.current_position[0])+'_'+str(self.current_position[1])+'_'+str(self.hd)+'.png', next_img)
            ...
            print('u=', ux, uy)
            self.ux_ls.append(ux)
            self.uy_ls.append(uy)
    ...
    def get_control_action(self,x_pos, y_pos, hd):

        # Move the camera to the new position
        nx = np.maximum(B, np.minimum((x_pos - x_size / 2) * 1000 + N / 2, B + W - 1))
        ny = np.maximum(B, np.minimum((y_pos - y_size / 2) * 1000 + N / 2, B + W - 1))
        nhd = (hd - 90) % 360
        self.camera.setPos(nx, ny , Z)
        self.camera.setHpr(nhd, P, 0)
        ...
        # print('*******image_shape0', image.shape)
        next_img = image[:, :, 2]
        plt.imsave('test/img/xyhd'+str(x_pos)+','+str(y_pos)+','+str(hd)+'.png', next_img)
        # print('*******next_image_shape0', next_img.shape)
        self.vismat.append(next_img.flatten())
        self.prev = next_img
        img = np.vstack(self.vismat)
