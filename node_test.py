...
class GazeboNeuralAnalysisNode(Node):
    ...
    def __init__(self, mode, cell_id_list=None, heading_list=None):
        ...
        self.current_image = np.array([[0,0]])  # Set by image_callback; guard in process_callback until first image
        ...
        try:
            ...
            if '/scan' in topic_names:
                ...
                try:
                    ...
                    # Verify our subscription (TopicEndpointInfo has node_name, topic_type, qos_profile; no topic_name)
                    sub_info = self.get_subscriptions_info_by_topic('/scan')
                    if sub_info:
                        info = sub_info[0]
                        self.get_logger().info(f"  Our subscription to /scan: node={getattr(info, 'node_name', '?')}, type={getattr(info, 'topic_type', '?')}")
                    else:
        ...
        try:
            ...
            if self.measurement_mode == 'neural_rate':
                ...
                if self.current_grid_occ is None:
                    self.get_logger().warn("No grid occupancy data available yet, using zero measurement")
                    return np.array([[0], [0]])
    ...
    def process_callback(self):
        ...
        if self.current_step == 0:
            ...
            try:
                # Wait for first image before processing
                # if self.current_image is None:
                #     self.get_logger().debug("No image available yet, skipping step")
                #     return
